...
static int sTokenCount;      /* 入力トークンカウント */
static int sTokenOffset[16]; /* トークン先頭オフセット配列 */
...
int task_command_parse(void)
{
    ...
    /* トークンに切り分ける */
    sTokenCount = strparse(sInputSerialData, sTokenOffset);
...
int task_command_excute(void)
{
    ...
    {
        com_puts(&sInputSerialData[sTokenOffset[i]]);
        com_putchar('\r');
        com_putchar('\n');
    }
    /* <---- テスト */
